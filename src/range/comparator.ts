import type { ParseBuild, ParsePre, PreId } from "../parser.js";
import type { IsNumericId } from "../string.js";
import type { CmpOp, PartialCore, PartialSemver, Wild } from "./ast.js";
...
export type ParsePartialCore<S extends string> = S extends `${infer A}.${infer R1}`
  ? R1 extends `${infer B}.${infer C}`
    ...
    : ToIdX<A> extends infer MA2 extends string | "x"
      ? ToIdX<R1> extends infer MI2 extends string | "x"
        ...
        : never
      : never
  : ToIdX<S> extends infer MA3 extends string | "x"
    ? { major: MA3 }
    : never;
...
export type ParsePartialSemver<S extends string> = S extends `${infer Left}+${infer Build}`
...
type MakeCmp<O extends CmpOp, S extends string> =
  ParsePartialSemver<Trim<S>> extends infer V extends PartialSemver
    ? { t: "cmp"; op: O; v: V }
    : never;
...
export type ParseComparator<S extends string> = S extends `^${infer VS}`
  ? MakeCaret<VS>
  : S extends `~${infer VS2}`
    ? MakeTilde<VS2>
    : S extends `>=${infer V1}`
