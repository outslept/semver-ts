...
import type { PreIdsToStr } from '../increment/pre-internals'
import type { BuildToStr } from '../increment/build'
import type { NextMajor, NextMinor, NextPatch } from '../increment/core-bumps'
import type { ComparatorSet, RangeAST, RangeNode, ComparatorNode, TildeNode, CaretNode, HyphenNode, PartialCore, PartialSemver } from './ast'
...
type CoreLower<C extends PartialCore> = `${ToNum<C['major']>}.${ToNum<GetMinor<C>>}.${ToNum<GetPatch<C>>}`
...
type HasWildcard<C extends PartialCore> =
  IsX<C['major']> extends true ? true :
    IsX<GetMinor<C>> extends true ? true :
      IsX<GetPatch<C>> extends true ? true : false
...
type ExpandGteWildcard<PS extends PartialSemver> =
  HasWildcard<PS['core']> extends true
    ? (IsX<PS['core']['major']> extends true ? [] : [{ op: '>='; v: PSLower<PS> }])
    : [{ op: '>='; v: PSLower<PS> }]

type ExpandGtWildcard<PS extends PartialSemver> =
  HasWildcard<PS['core']> extends true
    ? (UpperNextForWildcard<PS['core']> extends infer U extends string ? [{ op: '>='; v: U }] : [])
    : [{ op: '>'; v: PSLower<PS> }]
...
type RightIsFull<C extends PartialCore> =
  IsX<GetMinor<C>> extends true ? false :
    IsX<GetPatch<C>> extends true ? false : true
...
type ExpandNodes<Ns extends RangeNode[], Acc extends NormCmp[] = []> =
  Ns extends [infer H extends RangeNode, ...infer T extends RangeNode[]]
    ? (ExpandNode<H> extends infer E extends NormCmp[]
        ? ExpandNodes<T, Concat<Acc, E>>
        : never)
    : Acc

type ExpandSet<S extends ComparatorSet> =
  S['nodes'] extends infer Ns extends RangeNode[] ? ExpandNodes<Ns> : never

type MapSets<AST extends RangeAST, Acc extends NormSet[] = []> =
  AST extends [infer H extends ComparatorSet, ...infer T extends ComparatorSet[]]
