...
type CoreOf<V extends string> =
  ParseSemver<V> extends { major: infer A extends string; minor: infer B extends string; patch: infer C extends string } ? `${A}.${B}.${C}` : never

type CoreOfStr<S extends string> =
  ParseSemver<S> extends { major: infer A extends string; minor: infer B extends string; patch: infer C extends string } ? `${A}.${B}.${C}` : never

type MentionsCore<C extends NormCmp, K extends string> =
  ParseSemver<C['v']> extends { pre: infer P extends PreId[] }
    ? (P extends [] ? false : (CoreOfStr<C['v']> extends K ? true : false))
    : false

type SetMentionsCore<S extends NormSet, K extends string> =
  S extends [infer H extends NormCmp, ...infer T extends NormCmp[]]
    ? (MentionsCore<H, K> extends true ? true : SetMentionsCore<T, K>)
    : false
...
type SatisfiesSetRaw<V extends string, S extends NormSet> =
  S extends [infer H extends NormCmp, ...infer T extends NormCmp[]]
    ? (SatisfiesCmp<V, H> extends true ? SatisfiesSetRaw<V, T> : false)
    : true

type SatisfiesSet<V extends string, S extends NormSet, Include extends boolean> =
...
type SatisfiesAny<V extends string, R extends NormRange, Include extends boolean> =
  R extends [infer H extends NormSet, ...infer T extends NormSet[]]
    ? (SatisfiesSet<V, H, Include> extends true ? true : SatisfiesAny<V, T, Include>)
    : false

export type Satisfies<V extends string, R extends string, O extends { includePrerelease?: boolean } = {}> =
...
type MaxLB<A extends LB, B extends LB> =
  CmpSemver<A['v'], B['v']> extends infer C extends -1 | 0 | 1
    ? (C extends -1 ? B : (C extends 1 ? A : { kind: 'lb'; v: A['v']; strict: A['strict'] extends true ? true : B['strict'] extends true ? true : false }))
    : never
        ...
        : H['op'] extends '>'
          ...
          : H['op'] extends '<='
            ...
            : H['op'] extends '<'
              ...
              : H['op'] extends '='
                ? FoldBounds<T, L, U, (EqV extends string ? (CmpSemver<EqV, H['v']> extends 0 ? EqV : H['v']) : H['v'])>
                : never)
    : { lb: L; ub: U; eq: EqV }

type IntervalsIntersect<B1 extends { lb: LB; ub: UB | UBNone }, B2 extends { lb: LB; ub: UB | UBNone }> =
...
type IntersectsSetPair<S1 extends NormSet, S2 extends NormSet> =
  FoldBounds<S1> extends infer B1
    ? (B1 extends { lb: LB; ub: UB | UBNone; eq: infer E1 }
        ? (E1 extends string
            ? (SatisfiesSetRaw<E1, S2> extends true ? true : false)
            : (FoldBounds<S2> extends infer B2
                ? (B2 extends { lb: LB; ub: UB | UBNone; eq: infer E2 }
                    ? (E2 extends string
                        ? (SatisfiesSetRaw<E2, S1> extends true ? true : false)
...
type IntersectsAny<S1s extends NormSet[], S2s extends NormSet[]> =
  S1s extends [infer H1 extends NormSet, ...infer T1 extends NormSet[]]
    ? (S2s extends [infer H2 extends NormSet, ...infer T2 extends NormSet[]]
        ? (IntersectsSetPair<H1, H2> extends true ? true : (IntersectsAny<[H1], T2> extends true ? true : IntersectsAny<T1, S2s>))
        : false)
    : false
...
type SatisfiesRange<V extends string, NR extends NormRange> =
  SatisfiesAny<V, NR, true>
    ...
    ? (B extends { lb: LB; ub: UB | UBNone; eq: infer E1 }
        ? (E1 extends string
            ...
            : (SatisfiesRange<B['lb']['v'], R> extends true
                ? ([B['ub']] extends [UBNone] ? true : (B['ub'] extends UB ? (B['ub']['strict'] extends false ? SatisfiesRange<B['ub']['v'], R> : true) : never))
                : false))
        : never)
    : never

type SubsetAll<Sets extends NormSet[], R extends NormRange> =
  Sets extends [infer H extends NormSet, ...infer T extends NormSet[]]
    ? (SubsetSetInRange<H, R> extends true ? SubsetAll<T, R> : false)
    : true
